...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="distinctValues" as="xs:string*">
            <xsl:sequence select="distinct-values(//*/@xml:id[starts-with(., 'pmb')]/tokenize(., ' '))"/>
        </xsl:variable>
        ...
        <xsl:for-each select="$distinctValues">
            ...
            <xsl:text>&#x9;</xsl:text>
            <xsl:value-of select="concat('https://bahr-textverzeichnis.acdh.oeaw.ac.at/TSN-listperson.html#', .)"/>
