...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="root">
        ...
        <xsl:for-each
            ...
            <xsl:variable name="periodica-nummer"
                select="@ref"/>
                    ...
                    <text>
                        <body>
                            <xsl:element name="list" namespace="http://www.tei-c.org/ns/1.0">
                                <xsl:for-each
                                    select="collection(concat($folderURI, '../../data/editions/?select=HB-tv*.xml;recurse=yes'))[descendant::tei:biblStruct/tei:monogr/tei:title/@ref = $periodica-nummer]/node()">
                                    <xsl:element name="item" namespace="http://www.tei-c.org/ns/1.0">
                                        <xsl:value-of select="@xml:id"/>
                                        <xsl:copy-of select="descendant::tei:biblStruct/tei:analytic/tei:title" copy-namespaces="false"/>
                                        <xsl:copy-of select="descendant::tei:biblStruct/tei:monogr/descendant::tei:date[1]" copy-namespaces="false"/>
                                    </xsl:element>
                                </xsl:for-each>
                            </xsl:element>
                        </body>
